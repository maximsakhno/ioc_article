...
В качестве примера зависмости будем использовать логгер.
...
Одна из возможных реализаций логгера.
...
Одной из самых простых реализаций паттерна "локатор сервисов" является класс, хранящий объекты по ключам. В качестве ключей могут использоваться, например, строки.
...
В одном из модулей создадим глобальный зкземпляр, к которому будут обращаться все сущности нашего приложения, которым нужно зарегистрировать или разрешить зависимость.
...
Среди немногочисленных преимуществ данной реализации можно выделить отсутствие жесткой связности между компонентами. В то же время недостатки у такого подхода присутствуют в избытке, но обо всем попорядку.

Не всегда можно обойтись одним экземпляром по ключу. В нашем примере было бы удобно иметь возможность получать экземпляр логгера по имени. Такую проблему можно решить, если хранить не экземпляры классов, а фабрики для их получения. Этот подход значительно расширяет возможности по созданию зависимостей. Используя фабрики можно реализовать, как получение каждый раз единственного экземпляра класса, так и нового и вообще любую логику по разрешению зависимости. Реализация локатора сервисов примет следующий вид.
